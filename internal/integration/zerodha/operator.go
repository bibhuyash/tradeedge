...
// ExecuteStoredReadOnly runs only artifact-producing, read-only operations
// with a persisted V2 session. It cannot perform login or token exchange.
func ExecuteStoredReadOnly(args []string, sessionFile string, output, errorOutput io.Writer) int {
	if len(args) == 0 || (args[0] != "instrument-snapshot" && args[0] != "preflight") {
		_, _ = fmt.Fprintln(errorOutput, errInvalidCommand)
		return 1
	}
	if err := run(args, os.LookupEnv, output, commandDependencies{storedSessionFile: sessionFile}); err != nil {
...
// ExecuteCLI runs the legacy operator CLI over the shared read-only Zerodha
// operations. Authentication commands remain available to legacy operators;
// V2 preparation calls the stored-session entry points directly.
func ExecuteCLI(args []string, output, errorOutput io.Writer) int {
	return execute(args, os.LookupEnv, output, errorOutput, commandDependencies{})
}
...
func authenticate(args []string, lookup lookupEnv, output io.Writer, dependencies commandDependencies) error {
	...
	if dependencies.storedSessionFile == "" {
...
func preflight(args []string, lookup lookupEnv, output io.Writer, dependencies commandDependencies) error {
	...
	if options.credentialsFile != "" && dependencies.storedSessionFile == "" {
	...
	if failure, failed := preflightCredentialFailure(lookup); failed && dependencies.storedSessionFile == "" {
		if writeErr := writeAuthenticationFailure(output, failure); writeErr != nil {
			return writeErr
		}
		return errors.Join(errAuthentication, errDiagnosticReported)
	...
	if err != nil {
		failure := writePreflightFailure(output, "AUTHENTICATION", "ConfigurationError", "Invalid checksum-pinned runtime bundle", 0, 0, false)
		_, _ = fmt.Fprintln(output, "BUNDLE_ERROR="+bundleErrorClass(err))
		_, _ = fmt.Fprintln(output, "BUNDLE_DETAIL="+bundleErrorDetail(err))
		return failure
	}
...
func bundleErrorClass(err error) string {
	message := strings.ToLower(err.Error())
	for fragment, class := range map[string]string{
		"no such file": "FILE_MISSING", "cannot find the file": "FILE_MISSING",
		"file checksum mismatch": "CHECKSUM_MISMATCH", "invalid file reference": "INVALID_REFERENCE",
		"invalid runtime bundle": "INVALID_MANIFEST", "qualification": "INVALID_QUALIFICATION",
		"strategy": "INVALID_STRATEGY", "reference candidate": "INVALID_STRATEGY", "watchlist": "INVALID_WATCHLIST",
		"mapping": "INVALID_MAPPING", "instrument": "INVALID_INSTRUMENT_MASTER",
		"trailing json": "TRAILING_JSON",
	} {
...
func bundleErrorDetail(err error) string {
	message := strings.ToLower(err.Error())
	known := []string{
		"invalid runtime bundle", "shadow bundle requires qualification configurations",
		"paper bundle cannot authorize shadow qualification", "invalid file reference",
		"file checksum mismatch", "invalid instrument master", "instrument mapping not found",
		"ambiguous instrument mapping", "shadow bundle requires two reference candidates",
		"invalid shadow reference candidate", "invalid shadow qualification configuration",
		"invalid watchlist", "invalid watchlist mapping", "invalid market-data watchlist", "trailing json",
	}
	for _, candidate := range known {
		if strings.Contains(message, candidate) {
			return strings.ToUpper(strings.ReplaceAll(candidate, " ", "_"))
		}
	}
	for component, detail := range map[string]string{
		"calendar": "LOAD_CALENDAR_FAILED", "instrument_master": "LOAD_INSTRUMENT_MASTER_FAILED",
		"watchlist": "LOAD_WATCHLIST_FAILED", "strategies": "LOAD_STRATEGIES_FAILED",
		"portfolio": "LOAD_PORTFOLIO_FAILED", "risk": "LOAD_RISK_FAILED",
		"qualification_": "LOAD_QUALIFICATION_FAILED",
	} {
		if strings.Contains(message, "load "+component+":") {
			return detail
		}
	}
	return "UNCLASSIFIED"
...
func authenticatedSession(ctx context.Context, lookup lookupEnv, dependencies commandDependencies) (*brokerzerodha.SessionManager, error) {
	if dependencies.storedSessionFile != "" {
		return loadStoredSession(ctx, lookup, dependencies)
	}
	zerodhaConfig, err := loadReadOnlyConfig(lookup)
	if err != nil {
		return nil, err
...
func authenticatedBootstrapSession(ctx context.Context, lookup lookupEnv, dependencies commandDependencies) (*brokerzerodha.SessionManager, bool, error) {
	if dependencies.storedSessionFile != "" {
		session, err := loadStoredSession(ctx, lookup, dependencies)
...
func loadStoredSession(ctx context.Context, lookup lookupEnv, dependencies commandDependencies) (*brokerzerodha.SessionManager, error) {
	store, err := sessionfile.New(dependencies.storedSessionFile)
	if err != nil {
		return nil, err
	}
	...
	return brokerzerodha.NewStoredSessionManager(apiKey, record, operatorClock(dependencies))
}
